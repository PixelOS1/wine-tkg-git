...
EOM
	...
	# _nvidia pixelos
	if ["$_use_staging" = "true"]; then
		if ( cd "${srcdir}"/"${_stgsrcdir}" && git merge-base --is-ancestor 05d5df3292d6ae3ad5b2fa540bedbc011285c4d3 HEAD ); then
	...
	if ["$_use_staging" = "true"]; then
		if ( cd "${srcdir}"/"${_stgsrcdir}" && git merge-base --is-ancestor 279a1c527003811333646bcb827fda38618d8f21 HEAD ); then
...
EOM
	...
	if [ "$_EXTERNAL_INSTALL" = "proton" ] && [ "$_unfrog" != "true" ] && ! git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD || ([ "$_protonify" = "true" ] && git merge-base --is-ancestor 74dc0c5df9c3094352caedda8ebe14ed2dfd615e HEAD); then
	  if ( cd "${srcdir}"/"${_winesrcdir}" && git merge-base --is-ancestor 7ba3c31d84fcf49810aaf78aff63ef28b32eb159 HEAD ); then
	    ...
	    if git merge-base --is-ancestor ee2ef012f7466f6dcada1e8fe40da5132d4f1bb6 HEAD; then
	      _lastcommit="7ba3c31"
	      _rpc="1"
	      _stmbits="1"
